...
shinyServer(function(input, output) {
  ...
  model1 <- lm(Robbery~Aggravated.assault, data=VA_Murder)
  #model2 <- lm(Robbery~Aggravated.assault + Robbery, data=VA_Murder)
  ...
  yearselect <- reactive({
    ...
    sum(VA_Murder$Robbery[VA_Murder$Year <= yrInput])
  })
  bootstrap_it <- reactive({
    ...
    boot(data=VA_Murder, statistic=rsq, 
                    R=sample_num, formula=Robbery~Aggravated.assault)
  ...
  plot(VA_Murder$Aggravated.assault, VA_Murder$Robbery,
        xlab = "Aggravated Assault", ylab = "Robbery", 
        bty = "n", pch = 16)
  if(input$ShowModel1){
  ...
  observeEvent(input$show, {
    showModal(modalDialog(
      title = "Estimated Robbery in Virginia Help...",
      # tabPanel("Reference", 
      ...
      Use the slider for Aggravated Assault to run a linear model and predict the Estimated Robbery value. The predicted value associated with the model will appear under the plot. The Show/Hide Linear Model checkbox is also available to show or remove the linear model line in the plot.
      The Year slider can be used to calculate a cumulative sum of the estimated Robberies from the year 1960 to the selected year.
